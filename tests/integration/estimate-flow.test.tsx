...
describe("integration: estimate → pay → receipt happy path", () => {
  ...
  it("drives the chat, sends the estimate, pays via mock checkout, and flips status", async () => {
    ...
    await user.click(await screen.findByRole("button", { name: /wood privacy/i }, LONG_WAIT));
    await user.click(
    ...
    const nameInput = await screen.findByPlaceholderText(/^client name$/i, undefined, LONG_WAIT);

    // Send the estimate.
    await user.type(nameInput, "Integration Test Client");
    await user.type(screen.getByPlaceholderText(/^client email$/i), "integration@test.example");
