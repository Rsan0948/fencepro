...
const MOCK_PROJECTS = [
  {
    ...
    paidAt: "2025-01-22",
    adjustments: [],
    notes: "Double gate on north side.",
  },
  {
    id: "p2",
    client: "High Plains Ranch",
    county: "Weld",
    fenceType: "split_rail",
    linearFeet: 640,
    heightFt: 4,
    materialCost: 5120,
    laborCost: 3840,
    ...
    paidAt: null,
    adjustments: [{ label: "Added 80ft west run", amount: 480 }],
    notes: "Access gate at mile marker 2.",
  },
  {
    id: "p3",
    client: "Kowalski Residence",
    county: "Jefferson",
    fenceType: "vinyl",
    linearFeet: 95,
    heightFt: 6,
    ...
    paidAt: null,
    adjustments: [],
    notes: "HOA requires white vinyl only.",
  },
  {
    id: "p4",
    ...
    paidAt: null,
    adjustments: [],
    notes: "Estimate sent, awaiting signature.",
  },
  {
    id: "p5",
    client: "Chen Property LLC",
    county: "Boulder",
    ...
    paidAt: "2024-12-29",
    adjustments: [{ label: "Added corner bracing", amount: 180 }],
    notes: "",
  },
  {
    id: "p6",
    client: "Teller County Farm",
    county: "Teller",
    fenceType: "split_rail",
    linearFeet: 880,
    heightFt: 3,
    ...
    paidAt: "2024-11-30",
    adjustments: [],
    notes: "3-rail. Livestock grade.",
  },
  {
    id: "p7",
    client: "Vasquez New Build",
    county: "Adams",
    fenceType: "wood_picket",
    linearFeet: 120,
    heightFt: 4,
    materialCost: 1080,
    laborCost: 1008,
...
const MOCK_PRICES = {
  ...
  chain_link: { "Line Post": 18.47, "Chain Link 50ft": 89.0, "Tension Wire": 12.99 },
...
const COUNTY_RATES = {
  Denver: { low: 28, high: 45 },
  Boulder: { low: 32, high: 52 },
  Jefferson: { low: 26, high: 42 },
  Arapahoe: { low: 25, high: 40 },
  "El Paso": { low: 22, high: 36 },
  Summit: { low: 38, high: 62 },
  Eagle: { low: 40, high: 66 },
  Weld: { low: 20, high: 34 },
...
function calcQuote({ fenceType, linearFeet, heightFt, employees, hourlyWage }) {
  ...
  if (fenceType === "wood_privacy") {
    const posts = sections + 1,
      ...
      conc = posts;
    mat =
      posts * prices["4x4x8 Post"] +
      rails * prices["2x4x8 Rail"] +
      boards * prices["1x6x6 Board"] +
      conc * prices["80lb Concrete"];
    breakdown = [
      { item: "4x4x8 Posts", qty: posts, unit: prices["4x4x8 Post"] },
      { item: "2x4x8 Rails", qty: rails, unit: prices["2x4x8 Rail"] },
      { item: "1x6x6 Boards", qty: boards, unit: prices["1x6x6 Board"] },
      { item: "Concrete bags", qty: conc, unit: prices["80lb Concrete"] },
    ];
  } else {
...
function HoverBtn({ children, onClick, primary, style = {} }) {
  ...
  return (
    <button
      ...
      style={{
        background: primary ? (h ? t.accent : t.accentDim) : h ? t.surfaceHigh : "transparent",
        border: `1px solid ${primary ? t.accent : t.border}`,
        color: primary ? (h ? t.bg : t.accent) : t.sub,
        borderRadius: 10,
        padding: "10px 18px",
        fontSize: 13,
        fontWeight: 600,
      ...
      >
        <div>
          ...
          <div style={{ color: t.muted, fontSize: 12, fontFamily: mono }}>
            {p.county} County · {fenceLabel(p.fenceType)} · {p.linearFeet} lin ft
          </div>
        </div>
        {p.status !== "paid" && (
          <HoverBtn primary onClick={() => onInvoice(p)}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 8 }}>
            {[
              ...
              { label: "Deposit Collected", val: "-" + fmt(p.depositPaid), color: t.success },
              {
                ...
                val: fmt(remaining),
                color: p.status === "paid" ? t.muted : t.warn,
              },
            ].map((r) => (
              <div
                key={r.label}
...
function Slider({ label, min, max, step, value, onChange, format }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      />
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        <span style={{ color: t.muted, fontSize: 11, fontFamily: mono }}>
...
const STAGES = [
  {
    id: "county",
    msg: "Hey! Let's build your estimate. Which Colorado county is the project in?",
    render: (next) => <CountyPicker onSelect={(v) => next(v, `${v} County`)} />,
  },
  {
    id: "fenceType",
    msg: "Great. What type of fence are you building?",
    render: (next) => <FencePicker onSelect={(v) => next(v, v.label)} />,
  },
  {
    id: "dimensions",
    ...
    if (next < STAGES.length) {
      ...
      setTimeout(() => {
        ...
        const q = calcQuote({
          ...
          linearFeet: ans.linearFeet,
          heightFt: ans.heightFt,
          employees: ans.employees,
          hourlyWage: ans.hourlyWage,
        });
      ...
      >
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div
                style={{ color: t.text, fontSize: 14, fontWeight: 700, letterSpacing: "-0.02em" }}
              >
                FencePro
              </div>
              <div
